...
const project = {
    updateStartProject: () => {
        try {
            const startProjectPath = path.join(__dirname, 'StartProject.js');
            const updatedStartProjectContent = `const { execSync } = require('child_process');
const fs = require('fs');
const path = require('path');
...
`;
            ...
            // Écriture de StartProject.js
            fs.writeFileSync(startProjectPath, updatedStartProjectContent);
            console.log('StartProject.js mis à jour sans création de GitHub.js/DiagGitHub.js et sans push');

            // Ajout et commit des modifications (pas de push)
            execSync('git add .', { stdio: 'inherit' });
            execSync('git commit -m "Mise à jour de StartProject.js sans GitHub.js/DiagGitHub.js ni push" --allow-empty', { stdio: 'inherit' });
            console.log('Modifications commitées localement');
        } catch (error) {
            console.error('Erreur lors de la mise à jour de StartProject.js :', error.message);
        }
    }
};

// Exécution
project.updateStartProject();